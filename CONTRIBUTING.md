# Contributing to MongrelDB Elixir

Thanks for taking the time to help the MongrelDB Elixir client. This document
...
The MongrelDB Elixir client uses a standard **fork, branch, pull request**
workflow on GitHub.

1. **Fork** [`visorcraft/MongrelDB-Elixir`](https://github.com/visorcraft/MongrelDB-Elixir)
   to your GitHub account.
2. **Clone** your fork and add the upstream remote:

   ```sh
   git clone git@github.com:<you>/MongrelDB-Elixir.git
   cd MongrelDB-Elixir
   git remote add upstream https://github.com/visorcraft/MongrelDB-Elixir.git
   ```
...
5. **Open a pull request** against `master` on `visorcraft/MongrelDB-Elixir`.
...
- **Version.** Elixir 1.14+. Do not drop the minimum casually.
- **Style.** Run `mix format` and keep `mix credo --strict` clean. Prefer
  `with` statements over nested `case`, and `snake_case` for functions.
- **Naming.** `snake_case` for functions and variables. Module names are
  `MongrelDB.*`.
- **Transport.** Keep transport-specific behavior behind the `:httpc` call
  site, and return the existing typed exceptions instead of bare
  `{:error, :something}` atoms.
- **Dependencies.** Prefer the standard library (`:inets`, `Base`, `Regex`).
...
- The MongrelDB Elixir client version (from `mix.exs`).
- Your Elixir version (`elixir --version`) and OS.
...
The MongrelDB Elixir client is dual-licensed under MIT OR Apache-2.0. By
