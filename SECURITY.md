...
This document describes the security properties of the MongrelDB Elixir client
...
- The native Condition API and query builder accept typed parameters
  (column IDs, typed values). There is no string interpolation and no SQL
...
- Server and network errors are mapped to the typed exception hierarchy
  (`AuthException`, `NotFoundException`, `ConstraintException`,
  `ConnectionException`, `QueryException`), not leaked as bare atoms.
- NaN and Infinity are rejected with a clear `QueryException` rather than
  corrupting data.

## Dependency security

The MongrelDB Elixir client has no runtime dependencies beyond Elixir and
Erlang/OTP's `:inets` (the `:httpc` module). Development dependencies
(`ex_doc`, `credo`) are MIT or ISC licensed. Report dependency vulnerabilities
...
- the MongrelDB Elixir client version, Elixir version, and OS,
